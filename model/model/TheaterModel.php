...
class TheaterModel {
    ...
    public function getTheaterByID($id) {
        $stmt = $this->conn->prepare("SELECT TheaterID, TheaterName,  Address, Phone, NumberOfRooms FROM thearter WHERE TheaterID=:TheaterID");
        $stmt->bindParam(':TheaterID', $id);
        $stmt->setFetchMode(PDO::FETCH_CLASS,'Theater');
        $stmt->execute();
        $theater = $stmt->fetchObject();
        if($theater!=null){
            echo json_encode(array("success"=>true,"theater"=>$theater));
        }else{
            echo json_encode(array("success"=>false,"error"=>"Theater không tồn tại"));
        }
    }
    ...
    public function addTheater(Theater $Theater){
        try{
            $stmt =$this->conn->prepare("INSERT INTO thearter (TheaterID, TheaterName,  Address, Phone, NumberOfRooms) VALUES (:TheaterID, :TheaterName, :Address, :Phone, :NumberOfRooms)");
...
public function deleteTheater($id) {
    try {
        $stmt = $this->conn->prepare("DELETE FROM thearter WHERE TheaterID = :TheaterID");
        $stmt->bindParam(':TheaterID', $id);
        $stmt->execute();
        if ($stmt->rowCount() > 0) {
            echo json_encode(array("success" => true));
...
public function updateTheater(Theater $Theater) {
    try {
        $stmt = $this->conn->prepare("UPDATE thearter SET TheaterName = :TheaterName, Address = :Address, Phone = :Phone, NumberOfRooms = :NumberOfRooms WHERE TheaterID = :TheaterID");
    ...
    public function getAllTheater(){
        $stmt = $this->conn->prepare("SELECT TheaterID, TheaterName,  Address, Phone,  NumberOfRooms FROM thearter");
        $stmt->execute();
        $Theaters = array();
        ...
        echo json_encode(array("success" => true, "list" => $Theaters));
    }
    
        // Phương thức sinh mã ID mới cho Theater
        public function createNewID() {
            $query = "SELECT TheaterID FROM thearter ORDER BY CAST(RIGHT(TheaterID, LENGTH(TheaterID) - 2) AS UNSIGNED) DESC LIMIT 1";
...
$temp = new Theater('TheaterName 3 neeeeeeeee', 'Address thearter 3', 'P003', 3, 'TH001');


echo (new TheaterModel())->getAllTheater();
?>